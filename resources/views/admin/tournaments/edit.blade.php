...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tournament.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tournaments.update", [$tournament->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.tournament.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $tournament->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('startdate') ? 'is-invalid' : '' }}" type="text" name="startdate" id="startdate" value="{{ old('startdate', $tournament->startdate) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('enddate') ? 'is-invalid' : '' }}" type="text" name="enddate" id="enddate" value="{{ old('enddate', $tournament->enddate) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('mens') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="mens" id="mens" value="1" {{ $tournament->mens || old('mens', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('womens') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="womens" id="womens" value="1" {{ $tournament->womens || old('womens', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('mix') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="mix" id="mix" value="1" {{ $tournament->mix || old('mix', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('city') ? 'is-invalid' : '' }}" type="text" name="city" id="city" value="{{ old('city', $tournament->city) }}">
                @if($errors->has('city'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('club') ? 'is-invalid' : '' }}" name="club_id" id="club_id">
                    @foreach($clubs as $id => $club)
                        <option value="{{ $id }}" {{ (old('club_id') ? old('club_id') : $tournament->club->id ?? '') == $id ? 'selected' : '' }}>{{ $club }}</option>
                    @endforeach
                </select>
                @if($errors->has('club'))
